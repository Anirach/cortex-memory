...
\title{CORTEX: Self-Evolving Cognitive Memory for Autonomous AI Agents}
...
%==============================================================
\begin{abstract}
Persistent memory remains a fundamental limitation of large language model (LLM)-based autonomous agents. Current memory systems for AI agents typically support a single memory type with flat retrieval, lacking the multi-layered cognitive structure that enables biological intelligence to consolidate, forget, self-correct, and evolve retrieval strategies over time. We present \textbf{CORTEX} (Cognitive Orchestrated Retrieval with Temporal EXperience), a nine-layer cognitive memory architecture that unifies four biologically-inspired memory types---working, episodic, semantic, and procedural---with a hippocampal hybrid search index, an Ebbinghaus forgetting curve, a sleep/dream consolidation cycle, a self-improvement engine, a genetic algorithm for retrieval strategy evolution, a meta-cognition layer with active gap filling, and bidirectional synchronization with Obsidian knowledge vaults. CORTEX operates fully offline with zero external API dependencies, persists all state in SQLite, and requires only Python and NumPy. A multi-backend embedding system with automatic detection enables optional semantic embeddings via sentence-transformers or ONNX Runtime while preserving zero-mandatory-dependency operation. Experimental evaluation on 132 unit tests demonstrates correctness across all subsystems, with sub-millisecond memory operations and convergent evolution of retrieval strategies within 10 generations. To our knowledge, CORTEX is the first architecture to combine all nine cognitive layers---including self-evolution via genetic algorithms and active knowledge gap filling---in a single, embeddable memory system for autonomous agents.

\keywords{Cognitive memory architecture \and Autonomous agents \and Self-evolving AI \and Knowledge management \and Memory consolidation \and Genetic algorithms}
\end{abstract}

%==============================================================
\section{Introduction}\label{sec:intro}

The emergence of LLM-based autonomous agents~\cite{weng2023agent,sumers2023cognitive} has created an urgent need for persistent, structured memory systems that go beyond simple vector stores. While human cognition benefits from multiple specialized memory systems---working memory for immediate context, episodic memory for experiences, semantic memory for facts, and procedural memory for skills~\cite{tulving1972episodic,squire2004memory,baddeley2000working}---most current AI agent memory systems implement only one or two of these types, with no mechanism for self-improvement or evolution.

Current approaches such as Mem0~\cite{mem0_2024}, MemGPT/Letta~\cite{packer2023memgpt}, and Zep~\cite{zep2024} provide valuable memory persistence but suffer from three key limitations: (1)~they lack multi-type memory organization inspired by cognitive science, (2)~they have no self-evolution mechanism for retrieval strategies, and (3)~they do not actively detect and fill knowledge gaps. Furthermore, no existing system provides bidirectional synchronization with personal knowledge management tools, creating a disconnect between agent memory and human-curated knowledge bases.

\textbf{Contributions.} We introduce CORTEX, a nine-layer cognitive memory architecture that addresses these gaps:
...
\begin{enumerate}[leftmargin=*]
    ...
    \item \textbf{Zero-mandatory-dependency offline operation} using SQLite with WAL mode and a multi-backend embedding system that auto-detects the best available provider (ONNX Runtime, sentence-transformers, or hash-based TF-IDF), enabling real semantic embeddings without requiring external APIs.
\end{enumerate}
...
CORTEX is implemented in Python 3.10+ with full type annotations across 12 modules (2,800+ lines of code). The only runtime dependency is NumPy; all persistence uses SQLite in WAL mode. The system is structured as a pip-installable package with a public API centered on the \texttt{CortexEngine} class.

\subsection{Test Suite Results}

We validated CORTEX with a comprehensive test suite of 132 tests organized across 7 test modules. All tests pass with zero failures:

\begin{table}[t]
\centering
\caption{Test suite results: 132 tests, all passing in 0.38 seconds.}\label{tab:tests}
{\small
...
\midrule
\textbf{Total} & \textbf{132} & \textbf{0.38s execution time} \\
\bottomrule
\end{tabularx}
}
\end{table}
...
We presented CORTEX, a nine-layer cognitive memory architecture for autonomous AI agents that unifies biologically-inspired multi-type memory with self-evolution, active gap filling, and bidirectional knowledge vault synchronization. The architecture implements working, episodic, semantic, and procedural memory types connected through a hippocampal hybrid search index; an Ebbinghaus forgetting curve with sleep/dream consolidation; a self-improvement engine; genetic algorithm-based retrieval strategy evolution; meta-cognition with confidence scoring; active gap filling with five strategies and a four-class taxonomy; and bidirectional Obsidian vault integration with PARA method detection and wikilink graph mapping.

Experimental evaluation demonstrates correctness across 132 tests with sub-millisecond core operations and convergent evolution within 10 generations. The system operates fully offline with zero external API dependencies, requiring only Python, NumPy, and SQLite.
